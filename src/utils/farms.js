...
async function getPoolLength() {
  ...
  const poolLength = await joe.poolLength();
  return poolLength;
}

async function getPoolInfo(poolIndex) {
  const provider = new ethers.providers.JsonRpcProvider(
    "https://api.avax.network/ext/bc/C/rpc"
  );
  const joe = new ethers.Contract(joeBoostedMasterChefAddr, joeABI, provider);
  const poolInfo = await joe.poolInfo(poolIndex);
  return poolInfo;
}
...
function calcBoostedAPR(
  ...
  const veJoeProperUnits = veJoe * 10 ** 18
  const userFarmFactor = Math.sqrt((userLiq * 10 ** 18) * veJoeProperUnits);
  const totalFarmFactor = poolInfo.totalFactor;
  const boostedJoePerSecond = joePerSecond * 0.5
  const numerator = (userFarmFactor * boostedJoePerSecond)
  const userBoostedRewardsPerSec = numerator / totalFarmFactor;
  const userBoostedRewardsPerYear = userBoostedRewardsPerSec * SECONDSPERYEAR;
  const boostedAPR =
    (((userBoostedRewardsPerYear / 10 ** 18) * joePriceInUSD) / totalDeposited) * 100;
  return boostedAPR;
}

function calcBaseAPR(p, baseJoeRewardsPerSec, joePriceInUSD) {
  const userLiq = p.totalSupply;
  const rewardsPerSec = (userLiq * baseJoeRewardsPerSec) / p.totalSupply;
  const baseJoeRewardsPerYear = rewardsPerSec * SECONDSPERYEAR;
  const baseJoeAPR =
    ((baseJoeRewardsPerYear * joePriceInUSD) / p.reserveUSD) * 100;
...
export { getPoolLength, getPoolInfo, joePerSec, calcBoostedAPR, calcBaseAPR };
